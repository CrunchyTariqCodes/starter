...
function reservationRequirements(req, res, next){
  ...
  if (!first_name || first_name.trim() === '') {
    next({ status: 400, message: "Must input a first_name." });
  } else if (!last_name || last_name.trim() === "") {
    next({ status: 400, message: "Must input a last_name." });
  } else if (!mobile_number || typeof(mobile_number) !== 'string') {
    next({ status: 400, message: "Must input a mobile_number" });
  } else if (!reservation_date || !Date.parse(reservation_date)) {
    next({ status: 400, message: "Must input a reservation_date." });
  } else if (!reservation_time || !reservation_time.match(time)) {
    next({ status: 400, message: "Must input a reservation_time." });
 ...
 function tuesdayAndFutureDate(req, res, next){
  ...
  if (Date.now() < new Date(reservation_date + ' ' + reservation_time)) {
    ...
    if (tuesday === 2){
      next({ status: 400, message: `Cannot make a reservation; closed on Tuesdays.` });
    }
    return next();
  } else {
